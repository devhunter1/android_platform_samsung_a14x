...
int validate_super_block(struct f2fs_sb_info *sbi, enum SB_ADDR sb_addr)
{
	...
	if (!sanity_check_raw_super(sbi->raw_super, sb_addr)) {
		...
		/* P221221-04407: Force to check fully one time after T OS */
#define SEC_API_LEVEL_T_OS (33)
#define SEC_API_LEVEL_U_OS (34)
		if ((sbi->raw_super->api_level != SEC_API_LEVEL_T_OS) && 
			(sbi->raw_super->api_level != SEC_API_LEVEL_U_OS)) {
			sbi->raw_super->api_level = (__u8)SEC_API_LEVEL_U_OS;
			c.auto_fix = 0;
			c.fix_on = 1;
			c.convert_immutable = true;
...
out:
	if (c.func == FSCK) {
		...
		ptr = (char *)cp_page;
		dump_buffer(ptr, 512);
		dump_buffer(ptr + BLOCK_SZ - 256, 256);
	}
	free(cp_page);
	return NULL;
}
...
int sanity_check_ckpt(struct f2fs_sb_info *sbi)
{
	...
	if (!user_block_count || user_block_count >=
		...
		ASSERT_MSG("\tWrong user_block_count(%"PRIu64")\n", user_block_count);
		...
		if (flag & (CP_FSCK_FLAG | CP_RESIZEFS_FLAG)) {
			...
			MSG(0, "Info: Fix wrong user_block_count in CP: (%"PRIu64") -> (%"PRIu64")\n",
...
static int is_empty_segment(struct f2fs_sb_info *sbi, struct seg_entry *se,
				unsigned int from)
{
	unsigned int i;
	for (i = from; i < sbi->blocks_per_seg; i++) {
		if (f2fs_test_bit(i, (const char *)se->cur_valid_map))
			return 0;
	}
...
bypass_ssa:
	...
	se = get_seg_entry(sbi, curseg->segno);
	if (c.zoned_model == F2FS_ZONED_HM ||
			is_empty_segment(sbi, se, curseg->next_blkoff))
	...
	FIX_MSG("Move curseg[%d] alloc_type:%s %x -> %x after %"PRIx64"\n",
		i, curseg->alloc_type == LFS ? "LFS" : "SSR",
		old_segno, curseg->segno, from);
}
