...
/* for sec options */
#define OPT_SEC_VERSION     (SEC_OPT_START)

/* fsck */
#define OPT_SHOW_USAGE      (SEC_OPT_START + 1)

/* resize */
#define OPT_FAIL_SAFE       (SEC_OPT_START + 1)

/* sload */
#define OPT_MAX_SECTORS     (SEC_OPT_START + 1)
...
void fsck_usage()
{
	...
	MSG(0, "\nwith following sec-specific options:\n");
	MSG(0, "  --sec-version        print the version number and exit to check sec build\n");
	MSG(0, "  --show-disk-usage    show file size and disk space used\n");
	exit(1);
}
...
void resize_usage()
{
	...
	MSG(0, "\nwith following sec-specific options:\n");
	MSG(0, "  --sec-version        print the version number and exit to check sec build\n");
	MSG(0, "  --fail-safe [0|1]    fail-safe resize (Does not move main blocks) [default:1]\n");
	exit(1);
}
...
void sload_usage()
{
	...
	MSG(0, "\nwith following sec-specific options:\n");
	MSG(0, "  --sec-version        print the version number and exit to check sec build\n");
	MSG(0, "  --max-sectors        maximum # of expandable sectors [default:0]\n");
	exit(1);
}
...
void f2fs_parse_options(int argc, char *argv[])
{
	...
	if (!strcmp("fsck.f2fs", prog)) {
		...
		struct option long_opt[] = {
			...
			{"sec-version", no_argument, 0, OPT_SEC_VERSION},
			{"show-disk-usage", no_argument, 0, OPT_SHOW_USAGE},
			{0, 0, 0, 0}
		};
		...
		struct option long_opt[] = {
			{"sec-version", no_argument, 0, OPT_SEC_VERSION},
			{"fail-safe", required_argument, 0, OPT_FAIL_SAFE},
			{0, 0, 0, 0}
		};
		...
		while ((option = getopt_long(argc, argv, option_string,
			...
			case OPT_SEC_VERSION:
				...
				exit(0);
			case OPT_FAIL_SAFE:
				c.fail_safe_resize = strtol(optarg, &endptr, 0);
				if (*endptr != '\0') {
					MSG(0, "\tError: invalid fail-safe option %s\n", optarg);
					resize_usage();
				}
				break;
			default:
...
#ifdef WITH_SLOAD
		const char *option_string = "cL:a:i:x:m:rC:d:f:p:s:St:T:X:VP";
		...
		struct option long_opt[] = {
			{"sec-version", no_argument, 0, OPT_SEC_VERSION},
			{"max-sectors", required_argument, 0, OPT_MAX_SECTORS},
			{0, 0, 0, 0}
		};
		...
		while ((option = getopt_long(argc, argv, option_string,
			...
			case OPT_SEC_VERSION:
				show_version(prog);
				exit(0);
			case OPT_MAX_SECTORS:
				c.target_sectors = strtoull(optarg, &endptr, 0);
				if (*endptr != '\0') {
					MSG(0, "\tError: invalid max-sectors %s\n", optarg);
					sload_usage();
				}
				break;
			default:
