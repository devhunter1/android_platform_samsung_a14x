		...
		struct node_info *ni, struct child_info *child_d)
{
	...
	bool immutable = i_flags & FS_IMMUTABLE_FL;
...
check_next:
	...
	/* check & convert compress released flag */
	if (c.convert_immutable && compressed && immutable) {
		FIX_MSG("ino[0x%x], blkaddr[0x%x] clear FS_IMMUTABLE_FL(%d) "
			"and set FI_COMPRESS_RELEASED(%d)", nid, ni->blk_addr,
			immutable, compr_rel);
		immutable = false;
		node_blk->i.i_flags &= ~cpu_to_le32(FS_IMMUTABLE_FL);
		need_fix = 1;
	...
	for (idx = 0; idx < addrs; idx++, child.pgofs++) {
		...
		if (c.convert_immutable && compr_rel && blkaddr == NEW_ADDR &&
...
int fsck_verify(struct f2fs_sb_info *sbi)
{
	...
	printf("[FSCK] Max image size: %"PRIu64" MB, Free space: %"PRIu64" MB\n",
					get_max_image_size(sbi) >> 20, free_blks >> 8);
	...
	if (force || (c.fix_on && f2fs_dev_is_writable())) {
		...
		if (c.abnormal_stop || c.fs_errors || c.convert_immutable)
			update_superblock(sb, SB_MASK_ALL);
