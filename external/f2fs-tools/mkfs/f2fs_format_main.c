...
#define OPT_NODE_COUNT      (SEC_OPT_START + 2)
...
static void mkfs_usage()
{
	...
	MSG(0, "  --max-node-count=#   allocate only # of nat entries for optimization\n");
...
static void f2fs_parse_options(int argc, char *argv[])
{
	...
	static const struct option long_opts[] = {
		...
		{"sec-version", no_argument, NULL, OPT_SEC_VERSION},
		{"max-sectors", required_argument, NULL, OPT_MAX_SECTORS},
		{"max-node-count", required_argument, NULL, OPT_NODE_COUNT},
		{NULL, 0, NULL, 0}
	};
	int32_t option=0;
	int val;
	...
	while ((option = getopt_long(argc,argv,option_string,long_opts,NULL)) != EOF) {
		...
		case OPT_MAX_SECTORS:
			...
			break;
		case OPT_NODE_COUNT:
			c.max_nat = strtol(optarg, &endptr, 0);
			if (*endptr != '\0') {
				MSG(0, "\tError: invalid max-node-count %s\n", optarg);
			}
			break;
		default:
