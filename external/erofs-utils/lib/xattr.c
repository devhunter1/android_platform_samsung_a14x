...
static struct xattr_item *erofs_xattr_table(const char *srcpath)
{
	char kvbuf_static[XATTR_TABLE_ITEM_MAX_LEN];
	char *kvbuf = NULL;
	char *fspath;
	int ret;
	unsigned int len[2] = {0};
	unsigned int xattr_index;
	...
	if (0 == xattr_table_setup(cfg.xattr_table, fspath, &xattr_index, kvbuf_static, len)) {
		free(fspath);
		kvbuf = malloc(len[0] + len[1]);
		if (!kvbuf)
			return ERR_PTR(-ENOMEM);

		memcpy(kvbuf, kvbuf_static, len[0] + len[1]);

		return get_xattritem(xattr_index, kvbuf, len);
	}
	free(fspath);
	return NULL;
}
